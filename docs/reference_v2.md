...
| _map_.**getCenterExtent**()<br />_map_.**setCenterExtent**([value]) | Array(number)   | [undefined, undefined, undefined, undefined] | Get/set the view center extent, in order to prevent the user to pan beyond some limits. The array is specified as [xMin, yMin, xMax, yMax]. Note that these limit values apply to the view center, not to the view borders. |
