...
//DOM element to hold viewer
let container_element = document.getElementById("viz-container");
...
let line_width = 0.0013; //GL.LINE height
...
//pixels occupied by elements outside of renderer container e.g header & footer
let screen_margins = {
  top: 0.056 * window.innerHeight,
  left: 0,
  right: 0,
  bottom: 0
};
...
let viz_width = window.innerWidth - (screen_margins.left + screen_margins.right); //account for additional DOM elements
let viz_height = window.innerHeight - (screen_margins.top + screen_margins.bottom);
let fov = 40;
...
let background_color = 0x000000; //0x97dbf2
...
labelRenderer.domElement.style.top = screen_margins.top + "px";
...
window.addEventListener("resize", () => {
  ...
  viz_width = window.innerWidth - (screen_margins.left + screen_margins.right);
  viz_height = window.innerHeight - (screen_margins.top + screen_margins + bottom);
